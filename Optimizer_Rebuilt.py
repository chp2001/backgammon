...
class PopulationManager:
    def __init__(self, parent=None):
        ...
        self.targetGames = len(AINames) * 3
    ...
    def getRatingPercentileAgnostic(self, ratio:float, below:bool=True): #ratio is a number between 0 and 1, we want to return the list of builders below that percentile
        ...
        if below:
            return builders[:amt]
        return builders[-amt:]
    def getBuildersNeedingGames(self, amt:int):
    ...
    def getBuildersToRepopulate(self)->int:
        ...
        if len(builders) < self.targetActiveBuilders:
            return max(random.randrange(0,10),self.targetActiveBuilders - len(builders))
        return random.randrange(0,10)
...
class BuilderPool:
    def __init__(self, filepath:str="Optimizer_Rebuilt/") -> None:
