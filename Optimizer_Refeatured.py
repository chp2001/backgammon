...
class BuilderPool:
    def __init__(self, filepath:str="Optimizer_Refeatured/") -> None:
...
def getBestFromUnfeaturedForSeed():
    filepath = "Optimizer_Unfeatured/"
    #read the builders.txt file
    lines = []
    with open(filepath+"builders.txt", "r") as f:
        lines = f.readlines()
    #get the builders
    builders = []
    for i in range(0, len(lines)):
        #each builder takes up 4 lines, with a blank line between
        #the first line is the name
        #the second line is the " (winW/lossL)" format
        #we dont care about the other two lines
        if i % 5 == 0:
            temp = {}
            temp["name"] = lines[i].strip()
            winline = lines[i+1].strip()
            winline = winline[1:-1]
            winline = winline.split("/")
            temp["win"] = int(winline[0][:-1])
            temp["loss"] = int(winline[1][:-1])
            builders.append(temp)
    #sort the builders by win/loss ratio
    builders = [i for i in builders if i["win"]+i["loss"] > 100]
    builders.sort(key=lambda x: x["win"]/(x["loss"]+x["win"]), reverse=True)
    #return the top 10
    for i in builders[:10]:
        print(i["name"]+"%0%0")
    #exit()
    return [i["name"]+"%0%0" for i in builders[:100]]
...
if __name__ == "__main__":
    ...
    #exit()
    if len(optPool.activeBuilders) == 0:
        seedPop = getBestFromUnfeaturedForSeed()
        #exit()
        for i in seedPop:
            optPool.addBuilder(optPool.createBuilder(i))
    while True:
        optPool.cycle()
